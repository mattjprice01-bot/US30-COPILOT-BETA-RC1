...
def score_orderflow(ctx: dict[str, Any] | None) -> dict[str, Any]:
    if not isinstance(ctx, dict):
        return {"available": False, "score": 0.0, "reasons": [], "configured": False, "status": "OFFLINE"}
    if not ctx.get("fresh"):
        return {
            "available": False, "score": 0.0, "reasons": [],
            "configured": bool(ctx.get("configured")),
            "status": ctx.get("status", "WAITING"),
            "error": ctx.get("error"),
            "source": ctx.get("source", "Databento YM · L1 (MBP-1)"),
    ...
    reasons=[]
    if score > .35: reasons.append("YM L1 order flow favours buyers")
    elif score < -.35: reasons.append("YM L1 order flow favours sellers")
    else: reasons.append("YM L1 order flow balanced")
    keep = (
        "depth_imbalance","delta_norm","absorption","liquidity_shift",
        "bid_size_l1","ask_size_l1","bid_size_change","ask_size_change",
        "buy_volume_window","sell_volume_window","events_window",
        "best_bid","best_ask","mid","microprice","spread",
        "source","age_seconds","configured","status",
    )
    return {"available": True, "score": round(score,3), "reasons":reasons, **{k:ctx.get(k) for k in keep}}
...
def aggregate(payload: dict[str, Any], strategy: str = "scalp", news_ctx: dict[str, Any] | None = None,
    ...
    return {
        ...
        "ym_futures":ym, "ym":ym, "orderflow":of,
