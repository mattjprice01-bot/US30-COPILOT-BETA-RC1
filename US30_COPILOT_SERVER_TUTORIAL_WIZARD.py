...
async function boot(){let r=await fetch('/auth/me');if(!r.ok){$('login').classList.remove('hidden');$('app').classList.add('hidden');drawAuth();return}let j=await r.json();user=j.user;$('login').classList.add('hidden');$('app').classList.remove('hidden');$('plan').textContent=user.plan||'BETA';$('accountName').textContent=(user.display_name||user.email)+' · '+(user.plan||'BETA');await load();await loadConnections();refreshPushStatus();maybeStartTour();clearInterval(loadTimer);clearInterval(notifTimer);loadTimer=setInterval(load,5000);notifTimer=setInterval(pollNotifications,5000)}
