...
BUILD_ID = "2026-09-03-stripe-reconcile-2"
...
def require_entitled_user_id(user_id: int) -> dict[str, Any]:
    ...
    with db() as con:
        ...
        sub_row = con.execute("SELECT * FROM user_subscriptions WHERE user_id=?", (user_id,)).fetchone()
    if not row:
        raise HTTPException(404, "Account not found")
    user = dict(row)
    sub = dict(sub_row) if sub_row else None
    if (user.get("plan") or "").upper() != "ALPHA":
        sub = _reconcile_stripe_subscription(user, force=False, fail_closed=True)
