...
BUILD_ID = "2026-09-03-entitlement-diagnostic-4"
...
def require_entitled_user_id(user_id: int) -> dict[str, Any]:
    ...
    entitled = _is_entitled(user, sub)
    print(
        f"[ENTITLEMENT-DIAG] uid={user_id} "
        f"user_plan={(user.get('plan') or 'NONE')} "
        f"local_status={((local or {}).get('status') or 'NONE')} "
        f"reconciled_status={((sub or {}).get('status') or 'NONE')} "
        f"stripe_sub_present={bool((sub or {}).get('stripe_subscription_id'))} "
        f"entitled={entitled}",
        flush=True,
    )
    if not entitled:
