...
def _vapid_keys(con: sqlite3.Connection | None = None) -> tuple[str, str]:
    """Return (private PEM, public P-256 base64url). Env keys win; otherwise persist generated keys in DB."""
    ...
    own = con is None
    con = con or db()
    rows = {r["k"]: r["v"] for r in con.execute("SELECT k,v FROM push_config WHERE k IN ('vapid_private','vapid_public')")}
    ...
    con.execute("INSERT OR REPLACE INTO push_config(k,v) VALUES('vapid_private',?),('vapid_public',?)", (priv, pub))
    con.commit()
    if own:
        con.close()
    return priv, pub
...
def _send_web_push(con: sqlite3.Connection, user_id: int, title: str, body: str, event_type: str) -> None:
    try:
        priv, _ = _vapid_keys(con)
    except Exception:
        return
    payload = json.dumps({"title": title, "body": body, "event_type": event_type, "url": "/"})
    rows = con.execute("SELECT id,subscription_json FROM push_subscriptions WHERE user_id=?", (user_id,)).fetchall()
    for r in rows:
        try:
            webpush(subscription_info=json.loads(r["subscription_json"]), data=payload, vapid_private_key=priv,
            ...
            if code in (404, 410):
                con.execute("DELETE FROM push_subscriptions WHERE id=?", (r["id"],))
        except Exception:
            pass
...
@app.get("/health")
def health():
    return {"ok":True,"service":APP_TITLE,"commercial":True,"registration":ALLOW_REGISTRATION,"app_secret_configured":APP_SECRET != "CHANGE-ME-COMMERCIAL-ALPHA"}
...
def connections(request: Request, session: str | None = Cookie(default=None)):
    ...
    return {"ok":True,"connections":{
        "tradingview":{"configured":True,"webhook_url":f"{base}/webhook/tradingview","token_tail":""},
...
async def tradingview_unsecured(request: Request):
    # Temporary single-user test mode: fixed webhook with no token.
    # To avoid routing one customer's feed into another account, this only works
    # while exactly one active user exists in the database.
    with db() as con:
    ...
    if len(users) != 1:
        raise HTTPException(409, "Unsecured webhook requires exactly one active user")
...
def push_test(session: str | None = Cookie(default=None)):
    u = require_user(session)
    with db() as con:
        _send_web_push(con, u["id"], "US30 Copilot push test", "Background push notifications are working on this device.", "TEST")
        con.commit()
    return {"ok": True}
...
@app.get("/promo.png")
def promo():
    path = BASE / "assets" / "US30_COPILOT_PROMO.png"
    if path.exists():
        return FileResponse(path, media_type="image/png")
    fallback = base64.b64decode(
        "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII="
    )
    return Response(content=fallback, media_type="image/png", headers={"Cache-Control":"no-store"})
...
async function enablePush(){let st=$('pushStatus');if(!('Notification'in window)||!('serviceWorker'in navigator)||!('PushManager'in window)){st.textContent='Background push is not supported in this browser.';return}let p=await Notification.requestPermission();if(p!=='granted'){st.textContent='Notifications are blocked for this site.';return}try{let reg=await navigator.serviceWorker.ready;let sub=await reg.pushManager.getSubscription();if(!sub){let k=await(await fetch('/api/push/public-key',{cache:'no-store'})).json();sub=await reg.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:urlB64ToUint8Array(k.public_key)})}let r=await fetch('/api/push/subscribe',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(sub.toJSON())});if(!r.ok)throw new Error('subscription failed');localStorage.setItem('pushEnabled','1');st.textContent='Background push enabled on this device.';await testPush()}catch(e){console.error(e);st.textContent='Push setup failed: '+e.message}}
async function testPush(){try{let r=await fetch('/api/push/test',{method:'POST'});if(!r.ok)throw new Error('test failed');toast('Push test sent')}catch(e){toast('Push test failed')}}
async function refreshPushStatus(){let st=$('pushStatus');if(!st)return;if(!('Notification'in window)){st.textContent='Notifications not supported in this browser.';return}if(Notification.permission!=='granted'){st.textContent='Not enabled on this device.';return}try{let reg=await navigator.serviceWorker.ready,sub=await reg.pushManager.getSubscription();st.textContent=sub?'Background push enabled on this device.':'Permission granted — press Enable background push to finish setup.';if(sub)localStorage.setItem('pushEnabled','1')}catch(e){}}
...
@app.get("/sw.js")
def sw():
    js=r"""const C='us30-commercial-push-v2';
self.addEventListener('install',e=>{self.skipWaiting();e.waitUntil(caches.open(C).then(c=>c.addAll(['/','/icon.svg'])))});
self.addEventListener('activate',e=>e.waitUntil(self.clients.claim()));
self.addEventListener('fetch',e=>{if(e.request.method==='GET'&&!e.request.url.includes('/api/')&&!e.request.url.includes('/auth/'))e.respondWith(fetch(e.request).catch(()=>caches.match(e.request))) });
self.addEventListener('push',e=>{let d={};try{d=e.data?e.data.json():{}}catch(_){d={body:e.data?e.data.text():''}};e.waitUntil(self.registration.showNotification(d.title||'US30 Copilot',{body:d.body||'New Copilot alert',icon:'/icon.svg',badge:'/icon.svg',tag:'copilot-'+(d.event_type||'alert'),renotify:true,data:{url:d.url||'/'}}))});
...
"""
    return Response(content=js,media_type="application/javascript",headers={"Service-Worker-Allowed":"/","Cache-Control":"no-store"})
