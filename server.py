...
def connections(request: Request, session: str | None = Cookie(default=None)):
    ...
    if not token:
        ...
        c = _connection(u["id"])
    print(
        f"[TV-CONNECTIONS] db={DB} exists={DB.exists()} uid={u['id']} "
    ...
    with db() as con:
        ...
        if bar:
            try:
                ...
                if ts_ms > 0:
                    con.execute(
                        "INSERT OR REPLACE INTO candles_1m(user_id,ts,symbol,o,h,l,c,vol) VALUES(?,?,?,?,?,?,?,?)",
...
async def tradingview(token: str, request: Request):
    ...
    with db() as con:
        ...
        if not c:
            ...
            print(
                f"[TV-WEBHOOK-MISS] db={DB} exists={DB.exists()} "
        ...
        uid = int(c["user_id"])
        print(
            f"[TV-WEBHOOK-HIT] db={DB} uid={uid} incoming_tail={incoming[-6:]}",
            flush=True,
        )
