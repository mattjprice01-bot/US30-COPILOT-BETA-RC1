...
def _process_copilot(con: Any, user_id: int, payload: dict[str, Any], result: dict[str, Any]) -> None:
    ...
    if sess["status"] == "ARMED" and result.get("signal") == sess.get("side") and assess["quality"] == "READY" and assess["inside_entry"]:
        ...
        vt=_ensure_validation_trade(con,user_id,sess,result,px)
        if vt: _advance_validation_trade(con,vt,_one_minute_bar(payload) or {"h":px,"l":px},px)
    if sess["status"] != "LIVE":
        return
    side = sess.get("side")
    ...
    vt_row=con.execute("SELECT * FROM trades WHERE copilot_session_id=? LIMIT 1",(sess["id"],)).fetchone()
    if vt_row: _advance_validation_trade(con,dict(vt_row),bar,px)
...
def register(body: RegisterBody):
    ...
    with db() as con:
        try:
            cur = con.execute("INSERT INTO users(email,password_hash,password_salt,display_name,created_at) VALUES(?,?,?,?,?) RETURNING id", (email, _pwd_hash(body.password, salt), salt.hex(), body.display_name.strip(), now_iso()))
        except (sqlite3.IntegrityError, UniqueViolation):
...
def candles(tf: str = "1m", limit: int = 140, session: str | None = Cookie(default=None)):
    u = require_user(session); uid = int(u["id"])
...
def performance(session: str | None = Cookie(default=None)):
    ...
    return {"ok":True,"strategy":strategy,"tracked":len(rows),"closed":len(closed),"tp1_hit_rate":tp1_rate,"profit_factor":profit_factor,"expectancy_r":expectancy}
...
function drawCandles(){let cv=$('candleCanvas'),empty=$('chartEmpty');if(!cv)return;let bars=chartCandles||[];if(empty)empty.classList.toggle('hidden',bars.length>1);if(bars.length<2)return;let rect=cv.getBoundingClientRect(),dpr=Math.max(1,Math.min(2,window.devicePixelRatio||1)),w=Math.max(320,rect.width),h=Math.max(240,rect.height);cv.width=Math.round(w*dpr);cv.height=Math.round(h*dpr);let x=cv.getContext('2d');x.setTransform(dpr,0,0,dpr,0,0);x.clearRect(0,0,w,h);let pad={l:12,r:70,t:16,b:26},pw=w-pad.l-pad.r,ph=h-pad.t-pad.b;let levels=[],d=latest||{},directional=d.signal==='LONG'||d.signal==='SHORT';if(directional){['entry_low','entry_high','stop','tp1','tp2'].forEach(k=>{let v=Number(d[k]);if(Number.isFinite(v)&&v>0)levels.push(v)})}let lo=Math.min(...bars.map(b=>b.l),...(levels.length?levels:[Infinity])),hi=Math.max(...bars.map(b=>b.h),...(levels.length?levels:[-Infinity]));let span=Math.max(1,hi-lo);lo-=span*.07;hi+=span*.07;span=hi-lo;let py=v=>pad.t+(hi-v)/span*ph,px=i=>pad.l+(i+.5)/bars.length*pw;x.strokeStyle='rgba(91,142,184,.12)';x.lineWidth=1;x.font='10px system-ui';x.fillStyle='#718ba5';x.textAlign='left';for(let i=0;i<=5;i++){let yy=pad.t+i*ph/5,v=hi-i*span/5;x.beginPath();x.moveTo(pad.l,yy);x.lineTo(w-pad.r,yy);x.stroke();x.fillText(v.toLocaleString(undefined,{maximumFractionDigits:1}),w-pad.r+8,yy+3)}let bw=Math.max(2,Math.min(9,pw/bars.length*.62));bars.forEach((b,i)=>{let xx=px(i),yo=py(b.o),yc=py(b.c),yh=py(b.h),yl=py(b.l),up=b.c>=b.o;x.strokeStyle=up?'#35e6a1':'#ff697b';x.fillStyle=up?'#20c98a':'#e65367';x.beginPath();x.moveTo(xx,yh);x.lineTo(xx,yl);x.stroke();x.fillRect(xx-bw/2,Math.min(yo,yc),bw,Math.max(1,Math.abs(yc-yo)))});function lineSeries(vals,color,width){x.strokeStyle=color;x.lineWidth=width;x.beginPath();vals.forEach((v,i)=>{let xx=px(i),yy=py(v);if(i===0)x.moveTo(xx,yy);else x.lineTo(xx,yy)});x.stroke()}lineSeries(emaSeries(bars,20),'rgba(36,190,255,.92)',1.5);lineSeries(emaSeries(bars,50),'rgba(255,190,79,.85)',1.4);function hline(v,label,color,dash=[]){if(!Number.isFinite(Number(v))||Number(v)<=0)return;let yy=py(Number(v));if(yy<pad.t-2||yy>h-pad.b+2)return;x.save();x.setLineDash(dash);x.strokeStyle=color;x.lineWidth=1.2;x.beginPath();x.moveTo(pad.l,yy);x.lineTo(w-pad.r,yy);x.stroke();x.setLineDash([]);x.font='900 9px system-ui';let tw=x.measureText(label).width+10;x.fillStyle='rgba(3,12,22,.94)';x.fillRect(w-pad.r-tw-3,yy-9,tw,16);x.fillStyle=color;x.textAlign='right';x.fillText(label,w-pad.r-7,yy+3);x.restore()}if(directional&&Number.isFinite(Number(d.entry_low))&&Number.isFinite(Number(d.entry_high))){let y1=py(Math.max(Number(d.entry_low),Number(d.entry_high))),y2=py(Math.min(Number(d.entry_low),Number(d.entry_high)));x.fillStyle=d.signal==='LONG'?'rgba(49,230,160,.09)':'rgba(255,102,122,.09)';x.fillRect(pad.l,y1,pw,Math.max(2,y2-y1));hline(d.entry_low,'ENTRY','rgba(99,209,255,.82)',[4,4]);hline(d.entry_high,'ENTRY','rgba(99,209,255,.82)',[4,4]);hline(d.stop,'INVALID','rgba(255,102,122,.96)',[7,4]);hline(d.tp1,'TP1','rgba(49,230,160,.96)',[5,4]);hline(d.tp2,'TP2','rgba(49,230,160,.96)',[5,4]);let i=bars.length-1,xx=px(i),yy=py(bars[i].c);x.fillStyle=d.signal==='LONG'?'#31e6a0':'#ff667a';x.font='1000 10px system-ui';x.textAlign='right';x.fillText(d.signal,xx-8,Math.max(pad.t+10,yy-10))}let cp=bars[bars.length-1].c;hline(cp,'PRICE '+Number(cp).toLocaleString(undefined,{maximumFractionDigits:1}),'rgba(224,240,255,.72)',[2,4]);x.fillStyle='#637f99';x.font='9px system-ui';x.textAlign='center';let step=Math.max(1,Math.floor(bars.length/5));for(let i=0;i<bars.length;i+=step){let dt=new Date(bars[i].t);let lab=chartTf==='1m'||chartTf==='5m'||chartTf==='15m'||chartTf==='30m'?dt.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}):dt.toLocaleDateString([],{day:'2-digit',month:'short'});x.fillText(lab,px(i),h-8)}}
