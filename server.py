...
from shadow_mirror import mirror_packet
...
BUILD_ID = "2026-09-13-rc1-continuous-validation-rework"
...
# repository fallback secret. APP_SECRET protects provider credentials and reset tokens.
if STRIPE_SECRET_KEY and APP_SECRET == "CHANGE-ME-COMMERCIAL-ALPHA":
    raise RuntimeError("APP_SECRET must be set to a strong unique value before Stripe/commercial mode is enabled")

# Lightweight per-process auth throttling. This is deliberately independent of
# subscription state and does not reveal whether an account exists.
_AUTH_RATE_LOCK = threading.RLock()
_AUTH_RATE: dict[str, list[float]] = {}

def _auth_rate_limit(bucket: str, identity: str, *, limit: int, window_seconds: int) -> None:
    key = f"{bucket}:{identity.strip().lower()}"
    now = time.monotonic()
    cutoff = now - window_seconds
    with _AUTH_RATE_LOCK:
        hits = [t for t in _AUTH_RATE.get(key, []) if t > cutoff]
        if len(hits) >= limit:
...
# Stripe is authoritative for paid BETA access. Reconcile at controlled
# intervals so a missed/delayed webhook cannot leave stale local entitlement.
# The TradingView ingest path uses the cache, avoiding a Stripe API call per bar.
STRIPE_RECONCILE_SECONDS = max(30, int(os.getenv("STRIPE_RECONCILE_SECONDS", "300")))
_STRIPE_RECONCILE_LOCK = threading.RLock()
_STRIPE_RECONCILE_LAST: dict[int, float] = {}
...
CREATE TABLE IF NOT EXISTS password_reset_tokens(
  token_hash TEXT PRIMARY KEY,user_id BIGINT NOT NULL REFERENCES users(id),created_at TEXT NOT NULL,expires_at TEXT NOT NULL,used_at TEXT);
...
class PGCompatConnection:
    ...
    def executemany(self, sql: str, params_seq):
        # psycopg Connection exposes execute(), but bulk executemany() belongs
        # to a Cursor. Keep the SQLite-compatible application interface here.
...
def _billing_status(user: dict[str, Any]) -> dict[str, Any]:
    sub = _reconcile_stripe_subscription(user, force=True, fail_closed=False)
    return {
...
def _stripe_plain(obj: Any) -> dict[str, Any]:
    """Normalize Stripe SDK resources to a plain dict."""
    if obj is None:
        return {}
...
def _reconcile_stripe_subscription(
    ...
    *,
    ...
    """Refresh BETA access from Stripe without a Stripe call per market packet."""
    uid = int(user["id"])
    if (user.get("plan") or "").upper() == "ALPHA":
        return _subscription_row(uid)

    local = _subscription_row(uid)
    if not STRIPE_SECRET_KEY or not local:
        return local

    sub_id = str(local.get("stripe_subscription_id") or "").strip()
    customer_id = str(local.get("stripe_customer_id") or "").strip()
    if not sub_id and not customer_id:
        return local

    now_mono = time.monotonic()
    with _STRIPE_RECONCILE_LOCK:
        last = _STRIPE_RECONCILE_LAST.get(uid, 0.0)
        if not force and now_mono - last < STRIPE_RECONCILE_SECONDS:
            return local
        _STRIPE_RECONCILE_LAST[uid] = now_mono

    try:
        remote: dict[str, Any] = {}
        if sub_id:
            remote = _stripe_plain(stripe.Subscription.retrieve(sub_id))
        else:
            listing = stripe.Subscription.list(customer=customer_id, status="all", limit=10)
            candidates = [_stripe_plain(x) for x in (getattr(listing, "data", None) or _stripe_plain(listing).get("data") or [])]
            if STRIPE_BETA_PRICE_ID:
                candidates = [
                    x for x in candidates
                    if any(
                        str(((item.get("price") or {}).get("id")) or "") == STRIPE_BETA_PRICE_ID
                        for item in (((x.get("items") or {}).get("data")) or [])
                    )
                ]
            if candidates:
                remote = max(candidates, key=lambda x: int(x.get("created") or 0))

        if not remote:
            with db() as con:
                _upsert_subscription(con, uid, status="canceled", cancel_at_period_end=False)
                con.commit()
            print(f"[STRIPE-RECONCILE] uid={uid} local={local.get('status')} remote=MISSING -> canceled", flush=True)
            return _subscription_row(uid)

        items = (((remote.get("items") or {}).get("data")) or [])
        price_id = ((items[0].get("price") or {}).get("id")) if items else None
        status = str(remote.get("status") or "inactive")
        with db() as con:
            _upsert_subscription(
                con,
                uid,
                plan=(remote.get("metadata") or {}).get("plan") or local.get("plan") or "BETA",
                status=status,
                customer_id=str(remote.get("customer")) if remote.get("customer") else customer_id or None,
                subscription_id=str(remote.get("id")) if remote.get("id") else sub_id or None,
                price_id=price_id or local.get("stripe_price_id"),
                current_period_end=_stripe_dt(remote.get("current_period_end")),
                cancel_at_period_end=bool(remote.get("cancel_at_period_end")),
            )
            con.commit()

        if status != str(local.get("status") or "") or bool(remote.get("cancel_at_period_end")) != bool(local.get("cancel_at_period_end")):
            print(
                f"[STRIPE-RECONCILE] uid={uid} local={local.get('status')} remote={status} "
                f"cancel_at_period_end={bool(remote.get('cancel_at_period_end'))}",
                flush=True,
            )
        return _subscription_row(uid)
    except Exception as exc:
        with _STRIPE_RECONCILE_LOCK:
            _STRIPE_RECONCILE_LAST.pop(uid, None)
        print(f"[STRIPE-RECONCILE-ERROR] uid={uid} {type(exc).__name__}: {exc}", flush=True)
        if fail_closed:
            raise HTTPException(503, "Unable to verify subscription with Stripe") from exc
        return local
...
def require_entitled_user(session: str | None) -> dict[str, Any]:
    ...
    sub = _reconcile_stripe_subscription(u, force=False, fail_closed=True)
    if not _is_entitled(u, sub):
        raise HTTPException(402, "Active US30 Copilot BETA subscription required")
    return u


def require_entitled_user_id(user_id: int) -> dict[str, Any]:
    """Require commercial access for a user resolved without a login session."""
    with db() as con:
        row = con.execute("SELECT * FROM users WHERE id=? AND is_active=1", (user_id,)).fetchone()
        local_row = con.execute("SELECT * FROM user_subscriptions WHERE user_id=?", (user_id,)).fetchone()
    if not row:
        raise HTTPException(404, "Account not found")
    user = dict(row)
    local = dict(local_row) if local_row else None

    # Webhook ingestion is a server-to-server paid boundary. Reconcile Stripe
    # immediately here rather than trusting a process-local cache that may have
    # been populated before a cancellation/expiry was observed.
    sub = _reconcile_stripe_subscription(
        user,
        force=((user.get("plan") or "").upper() != "ALPHA"),
        fail_closed=True,
    )
    entitled = _is_entitled(user, sub)
    if not entitled:
        raise HTTPException(403, "US30 Copilot subscription inactive")
    return user
...
def _entry_assessment(result: dict[str, Any], sess: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    # Confirmation-quality caps. TradingView remains the core engine and FRED
    # supplies macro/event context; Databento/YM confirmation increases
    # confidence rather than being mandatory for a valid setup.
    ym = result.get("ym") or {}
    orderflow = result.get("orderflow") or {}
    ym_available = bool(ym.get("available"))
    l1_available = bool(orderflow.get("available"))
    if signal in ("LONG", "SHORT") and not result.get("news_block"):
        if not ym_available and not l1_available:
            score = min(score, 85)
            reasons.append("technical/macro setup; no YM or L1 confirmation")
        elif ym_available != l1_available:
            score = min(score, 95)
            reasons.append("one futures confirmation layer available")
        else:
            reasons.append("full futures confirmation stack available")
...
def _ensure_continuous_validation_session(con: Any, user_id: int, result: dict[str, Any]) -> dict[str, Any]:
    ...
    # Once an objective validation trade closes, archive its plan and begin a
    # fresh scanning cycle immediately. Manual LIVE sessions are never touched.
    if sess and sess.get("status") == "ARMED":
        tracked = con.execute(
            "SELECT status FROM trades WHERE copilot_session_id=? ORDER BY id DESC LIMIT 1",
            (sess["id"],),
        ).fetchone()
    ...
    if not sess or sess.get("status") == "IDLE":
        if sess:
            con.execute(
                "UPDATE copilot_sessions SET status='CLOSED',updated_at=?,closed_at=?,close_reason='AUTO_REARM' WHERE id=?",
                (now, now, sess["id"]),
            )
        cfg = get_strategy(result.get("strategy"))
        cur = con.execute(
            """INSERT INTO copilot_sessions(user_id,created_at,updated_at,status,strategy,symbol)
               VALUES(?,?,?,?,?,?) RETURNING id""",
            (user_id, now, now, "LOOKING", cfg.name, result.get("symbol") or "US30"),
        )
...
def _process_copilot(con: Any, user_id: int, payload: dict[str, Any], result: dict[str, Any]) -> None:
    ...
    sess = _ensure_continuous_validation_session(con, user_id, result)
...
def _send_password_reset_email(email: str, reset_url: str) -> None:
    ...
    payload=json.dumps({"from":RESET_EMAIL_FROM,"to":[email],"subject":"Reset your US30 Copilot password","html":(
        "<div style='font-family:Arial,sans-serif;max-width:560px;margin:auto;color:#142033'>"
        "<h2>Reset your US30 Copilot password</h2><p>We received a request to reset the password for your account.</p>"
        f"<p><a href='{reset_url}' style='display:inline-block;padding:12px 18px;background:#087bdc;color:white;text-decoration:none;border-radius:8px;font-weight:bold'>Reset password</a></p>"
        f"<p>This link expires in {PASSWORD_RESET_MINUTES} minutes and can be used once.</p>"
        "<p>If you did not request this, you can ignore this email.</p>"
        "<p style='font-size:12px;color:#667'>US30 Copilot · decision-support software</p></div>"
    )}).encode()
    req=urllib.request.Request("https://api.resend.com/emails",data=payload,method="POST",headers={"Authorization":f"Bearer {RESEND_API_KEY}","Content-Type":"application/json","User-Agent":"US30-Copilot/1.0"})
    try:
...
def register(body: RegisterBody):
    _auth_rate_limit("register", body.email, limit=5, window_seconds=3600)
...
def login(body: LoginBody):
    _auth_rate_limit("login", body.email, limit=10, window_seconds=900)
    with db() as con:
...
def forgot_password(body: ForgotPasswordBody, request: Request):
    _auth_rate_limit("forgot", body.email, limit=4, window_seconds=3600)
    public={"ok":True,"message":"If that email is registered, a password reset link has been sent."}
    email=body.email.lower().strip()
    with db() as con:
        row=con.execute("SELECT id,email FROM users WHERE email=? AND is_active=1",(email,)).fetchone()
        if not row: return public
        uid=int(row["id"])
        con.execute("UPDATE password_reset_tokens SET used_at=? WHERE user_id=? AND used_at IS NULL",(now_iso(),uid))
        raw_token=secrets.token_urlsafe(48)
        expires=datetime.now(timezone.utc)+timedelta(minutes=PASSWORD_RESET_MINUTES)
        con.execute("INSERT INTO password_reset_tokens(token_hash,user_id,created_at,expires_at,used_at) VALUES(?,?,?,?,NULL)",(_reset_token_hash(raw_token),uid,now_iso(),expires.isoformat()))
        con.commit()
    base=PUBLIC_URL or str(request.base_url).rstrip("/")
    try:
        _send_password_reset_email(email,f"{base}/?reset_token={raw_token}")
        print(f"[PASSWORD-RESET-REQUEST] uid={uid} delivery=accepted",flush=True)
    except Exception as exc:
        print(f"[PASSWORD-RESET-EMAIL-ERROR] uid={uid} {type(exc).__name__}: {exc}",flush=True)
    return public

@app.post("/auth/reset-password")
def reset_password(body: ResetPasswordBody):
    if len(body.password)<10: raise HTTPException(422,"Password must be at least 10 characters")
    token=(body.token or "").strip()
    if len(token)<32: raise HTTPException(400,"Invalid or expired reset link")
    with db() as con:
        row=con.execute("""SELECT pr.*,u.is_active FROM password_reset_tokens pr JOIN users u ON u.id=pr.user_id
                           WHERE pr.token_hash=? AND pr.used_at IS NULL AND pr.expires_at>? AND u.is_active=1""",(_reset_token_hash(token),now_iso())).fetchone()
        if not row: raise HTTPException(400,"Invalid or expired reset link")
        uid=int(row["user_id"]); salt=os.urandom(16)
        con.execute("UPDATE users SET password_hash=?,password_salt=? WHERE id=?",(_pwd_hash(body.password,salt),salt.hex(),uid))
        con.execute("UPDATE password_reset_tokens SET used_at=? WHERE user_id=? AND used_at IS NULL",(now_iso(),uid))
        con.execute("DELETE FROM auth_sessions WHERE user_id=?",(uid,))
        con.commit()
    print(f"[PASSWORD-RESET-COMPLETE] uid={uid}",flush=True)
...
def billing_portal(request: Request, session: str | None = Cookie(default=None)):
    ...
    if not STRIPE_SECRET_KEY:
        raise HTTPException(503, "Stripe billing is not configured yet")
    sub = _reconcile_stripe_subscription(u, force=True, fail_closed=False)
...
async def stripe_webhook(request: Request):
    ...
    # Stripe Python v12+ returns Event/StripeObject instances, not plain dicts.
    # Convert once at the webhook boundary so the rest of the handler can use
    # normal dict access regardless of SDK object type.
    ...
    if uid:
        with _STRIPE_RECONCILE_LOCK:
            _STRIPE_RECONCILE_LAST.pop(int(uid), None)
    return {"ok": True}

@app.get("/data-brain.png")
def data_brain_image():
    path = Path(__file__).resolve().with_name("US30_COPILOT_DATA_BRAIN.png")
    if not path.exists():
        raise HTTPException(404, "Data brain artwork not installed")
    return FileResponse(path, media_type="image/png")

@app.get("/api/connections")
def connections(request: Request, session: str | None = Cookie(default=None)):
    u = require_entitled_user(session); c = _connection(u["id"])
    base = PUBLIC_URL or str(request.base_url).rstrip("/")
    ...
    return {"ok":True,"connections":{
        ...
        "input_coverage":{
            "tradingview":33.3,
            "fred":33.3 if bool(_dec(c.get("fred_key_enc"))) else 0.0,
            "databento":33.4 if bool(_dec(c.get("databento_key_enc"))) else 0.0,
            "total":round(33.3 + (33.3 if bool(_dec(c.get("fred_key_enc"))) else 0.0) + (33.4 if bool(_dec(c.get("databento_key_enc"))) else 0.0), 1),
            "label":"FULL INPUT STACK" if bool(_dec(c.get("fred_key_enc"))) and bool(_dec(c.get("databento_key_enc"))) else ("TECHNICAL + MACRO" if bool(_dec(c.get("fred_key_enc"))) else "TECHNICAL CORE")
        },
        "databento":{
...
async def _ingest_tradingview_for_user(uid: int, request: Request):
    try:
        ...
        print(f"[TV-WEBHOOK-BLOCKED] backend={DB_BACKEND} uid={uid} reason=NOT_ENTITLED", flush=True)
        raise exc
    try:
...
def download_tradingview_pine(feed: str = "standard", session: str | None = Cookie(default=None)):
    require_entitled_user(session)
    if feed.lower() == "cme":
...
.tourOverlay{position:fixed;inset:0;z-index:9998;background:rgba(1,5,12,.82);backdrop-filter:blur(8px);display:grid;place-items:center;padding:18px}.tourCard{width:min(720px,100%);max-height:90vh;overflow:auto;padding:24px;border:1px solid rgba(52,183,255,.35);box-shadow:0 30px 100px rgba(0,0,0,.62),0 0 60px rgba(24,168,255,.10)}.tourTop{display:flex;justify-content:space-between;gap:14px;align-items:flex-start}.tourProgress{height:7px;background:#102238;border-radius:99px;overflow:hidden;margin:18px 0}.tourProgress>div{height:100%;background:linear-gradient(90deg,#087bdc,#31e6a0);transition:width .25s ease}.tourIcon{width:52px;height:52px;border-radius:16px;display:grid;place-items:center;font-size:25px;font-weight:1000;background:linear-gradient(145deg,#0a78ca,#12b6ff);box-shadow:0 0 25px rgba(20,175,255,.2)}.tourTitle{font-size:26px;font-weight:1000;margin:8px 0 5px}.tourBody{font-size:14px;line-height:1.65;color:#a9bdd1}.tourCheck{margin-top:14px;padding:13px 14px;border:1px solid rgba(76,147,205,.2);background:#061321;border-radius:14px}.tourCheck.good{border-color:rgba(49,230,160,.4);background:rgba(7,63,44,.25)}.tourActions{display:flex;justify-content:space-between;gap:9px;flex-wrap:wrap;margin-top:20px}.tourActions .right{display:flex;gap:8px;margin-left:auto}.tourSkip{background:transparent;border:0;color:#7992aa;cursor:pointer;font-weight:800}.helpBtn{border:1px solid rgba(69,153,218,.25);background:#081725;color:#9fdcff;border-radius:10px;padding:7px 10px;font-size:10px;font-weight:950;cursor:pointer}.tourMini{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:12px}.tourMini>div{padding:11px;border-radius:12px;background:#061321;border:1px solid rgba(65,127,184,.14);font-size:11px;color:#9eb4c9}.tourMini b{display:block;color:#eef8ff;margin-bottom:4px}@media(max-width:650px){.tourCard{padding:18px}.tourTitle{font-size:22px}.tourMini{grid-template-columns:1fr}.tourActions{display:block}.tourActions .right{margin-top:10px}.tourActions button{width:100%;margin-top:7px}}
.tvProgress{margin-top:14px;padding:13px 14px;border-radius:14px;background:#061321;border:1px solid rgba(65,141,198,.18)}.tvProgressTop{display:flex;justify-content:space-between;gap:12px;align-items:center}.tvProgressTop b{font-size:13px}.tvProgressTop span{font-size:11px;color:#8ea8c1}.tvProgressBar{height:7px;background:#102238;border-radius:99px;overflow:hidden;margin-top:9px}.tvProgressBar i{display:block;height:100%;width:0;background:linear-gradient(90deg,#087bdc,#31e6a0);border-radius:99px;transition:width .3s}.wizardStep.done{border-color:rgba(49,230,160,.42);background:rgba(7,63,44,.16)}.wizardStep.done .stepNo{background:linear-gradient(135deg,#11855f,#31e6a0);color:#03120c}.tvCallout{margin-top:9px;padding:10px 12px;border-left:3px solid #22b8ff;background:rgba(9,40,66,.42);border-radius:8px;font-size:11px;line-height:1.5;color:#a9c9df}.tvSuccess{font-weight:950;color:#6ef1bc;line-height:1.65}.tvTrouble{margin-top:9px;font-size:11px;line-height:1.55;color:#91a8c0}@media(max-width:650px){.feedChoice{grid-template-columns:1fr}.wizard{padding:14px}.wizardStep{grid-template-columns:30px 1fr}.stepNo{width:30px;height:30px}} 
/* RC1 professional terminal visual system */
body{background:
radial-gradient(circle at 12% 8%,rgba(12,111,180,.19),transparent 31%),
radial-gradient(circle at 88% 18%,rgba(0,210,196,.09),transparent 28%),
radial-gradient(circle at 54% 110%,rgba(42,69,151,.15),transparent 40%),#02060d;background-attachment:fixed}
body:before{opacity:.92;background-image:linear-gradient(rgba(79,163,218,.035) 1px,transparent 1px),linear-gradient(90deg,rgba(79,163,218,.035) 1px,transparent 1px);background-size:42px 42px;animation:gridDrift 24s linear infinite}
body:after{content:"";position:fixed;z-index:-1;inset:-28%;pointer-events:none;background:conic-gradient(from 210deg at 50% 50%,transparent,rgba(0,170,255,.055),transparent 23%,rgba(55,230,185,.035),transparent 47%);filter:blur(55px);animation:ambientOrbit 32s linear infinite}
.glass{backdrop-filter:blur(16px) saturate(1.08);-webkit-backdrop-filter:blur(16px) saturate(1.08);box-shadow:0 22px 65px rgba(0,0,0,.38),inset 0 1px rgba(255,255,255,.035),inset 0 0 45px rgba(28,108,167,.025)}
.top{padding:4px 2px}.miniLogo{overflow:hidden;animation:logoPulse 4.5s ease-in-out infinite}.miniLogo:before{content:"";position:absolute;inset:-45%;background:conic-gradient(transparent,#42d5ff,transparent 28%);animation:logoOrbit 5s linear infinite}.miniLogo:after{background:#06111d;inset:2px;border-radius:10px}
.pill.live{position:relative;overflow:hidden}.pill.live:after{content:"";position:absolute;inset:-1px;background:linear-gradient(110deg,transparent 20%,rgba(255,255,255,.18),transparent 48%);transform:translateX(-130%);animation:statusSheen 4.2s ease-in-out infinite}
.hero{background:radial-gradient(circle at 78% 0,rgba(24,148,218,.11),transparent 34%),linear-gradient(165deg,rgba(11,29,48,.97),rgba(3,10,19,.98))}.hero:before{animation:heroOrbit 18s linear infinite}.scanline{height:2px;box-shadow:0 0 16px rgba(40,191,255,.55)}
.metric,.context{transition:transform .22s ease,border-color .22s ease,background .22s ease}.metric:hover,.context:hover{transform:translateY(-2px);border-color:rgba(59,190,243,.32);background:rgba(8,26,43,.76)}
.radar{isolation:isolate;background:radial-gradient(circle,rgba(11,54,83,.92) 0,rgba(5,24,40,.96) 43%,#030c16 72%);box-shadow:inset 0 0 42px rgba(15,158,213,.12)}.radar:before,.radar:after{box-shadow:0 0 18px rgba(50,190,245,.035)}.radarCore:before{content:"";display:block;width:7px;height:7px;border-radius:50%;background:#4ee6b0;margin:0 auto 9px;box-shadow:0 0 16px #31e6a0;animation:corePing 2s ease-out infinite}.radarSweep{background:linear-gradient(90deg,rgba(38,195,255,.18),transparent);border-left-color:#44d7ff;filter:drop-shadow(0 0 10px #20baff)}
.coverageBrain:before{content:"";position:absolute;z-index:2;inset:10%;border:1px solid rgba(57,205,255,.25);border-radius:50%;box-shadow:0 0 28px rgba(40,191,255,.08),inset 0 0 28px rgba(40,191,255,.05);animation:brainOrbit 12s linear infinite}.coverageBrain:after{content:"";position:absolute;z-index:3;left:7%;right:7%;height:1px;top:15%;background:linear-gradient(90deg,transparent,#50ddff,transparent);box-shadow:0 0 13px #2cc8ff;animation:brainScan 4.4s ease-in-out infinite}.coverageBrain img{animation:brainBreathe 6s ease-in-out infinite}.coveragePct{z-index:4}
.chartCard{position:relative}.chartCard:before{content:"";position:absolute;inset:0;pointer-events:none;background:linear-gradient(115deg,transparent 0,rgba(74,205,255,.026) 35%,transparent 58%);animation:panelSheen 9s ease-in-out infinite}.chartTop,.chartLegend{position:relative;z-index:2}.chartWrap:before{content:"";position:absolute;z-index:0;inset:0;pointer-events:none;background-image:linear-gradient(rgba(86,151,194,.035) 1px,transparent 1px),linear-gradient(90deg,rgba(86,151,194,.035) 1px,transparent 1px);background-size:64px 52px}.chartTelemetry{display:flex;gap:7px;flex-wrap:wrap;margin-top:8px}.chartTelemetry span{padding:4px 7px;border-radius:6px;border:1px solid rgba(68,156,208,.18);background:rgba(3,13,23,.66);font:750 8px ui-monospace,monospace;letter-spacing:.8px;color:#7194ad}.chartTelemetry i{display:inline-block;width:5px;height:5px;border-radius:50%;background:#31e6a0;box-shadow:0 0 8px #31e6a0;margin-right:5px;animation:telemetryPulse 1.8s ease-in-out infinite}
.statebar button:disabled{cursor:default;opacity:.78}.statebar button.active{animation:activeState 2.8s ease-in-out infinite}
@keyframes gridDrift{to{background-position:42px 42px}}@keyframes ambientOrbit{to{transform:rotate(360deg)}}@keyframes logoOrbit{to{transform:rotate(360deg)}}@keyframes logoPulse{50%{box-shadow:0 0 30px rgba(24,168,255,.31)}}@keyframes statusSheen{0%,55%{transform:translateX(-130%)}80%,100%{transform:translateX(130%)}}@keyframes heroOrbit{to{transform:rotate(360deg)}}@keyframes corePing{0%{box-shadow:0 0 0 0 rgba(49,230,160,.65)}80%,100%{box-shadow:0 0 0 14px rgba(49,230,160,0)}}@keyframes brainOrbit{to{transform:rotate(360deg)}}@keyframes brainScan{0%,100%{top:15%;opacity:.15}50%{top:85%;opacity:.9}}@keyframes brainBreathe{50%{transform:scale(1.035);filter:saturate(1.14) brightness(1.04)}}@keyframes panelSheen{0%,40%{transform:translateX(-70%);opacity:0}65%{opacity:1}100%{transform:translateX(70%);opacity:0}}@keyframes telemetryPulse{50%{opacity:.3}}@keyframes activeState{50%{box-shadow:0 0 25px rgba(20,175,255,.24)}}
@media(prefers-reduced-motion:reduce){*,*:before,*:after{animation-duration:.001ms!important;animation-iteration-count:1!important;scroll-behavior:auto!important}}
</style></head><body>
<div id="login" class="login glass hidden"><div class="loginGrid"><div class="loginArt"></div><div class="loginForm"><div class="eyebrow">US30 COPILOT · DATA ENGINE</div><h1>Market intelligence.<br>Not another signal app.</h1><p>One decision engine combining your live technical feed, macro context and optional futures/order-flow confirmation.</p><div class="tabs"><button class="btn" onclick="mode='login';drawAuth()">Sign in</button><button class="btn ghost" onclick="mode='register';drawAuth()">Create account</button></div><input id="name" class="field" placeholder="Display name"><input id="email" class="field" placeholder="Email"><input id="password" class="field" type="password" placeholder="Password (10+ characters)"><input id="password2" class="field hidden" type="password" placeholder="Confirm new password"><button id="authBtn" class="btn" style="margin-top:10px" onclick="authSubmit()">Sign in</button><button id="forgotBtn" class="tourSkip" style="margin-top:12px" onclick="forgotPassword()">Forgot password?</button><button id="backLoginBtn" class="tourSkip hidden" style="margin-top:12px" onclick="backToLogin()">Back to sign in</button><div id="authErr" class="error"></div><div class="small">Decision-support software · no automatic trade execution · confidence is an engine score, not a guaranteed probability.</div></div></div></div>
...
<div id="dashboardPage"><div class="grid"><div class="hero glass"><div class="heroTop"><div><div id="symbol" class="eyebrow">WAITING FOR YOUR DATA FEED</div><div id="signal" class="sig WAIT">SCANNING</div><div id="confidence" class="confidence">Continuous validation armed</div><div id="summary" class="sub">Connect TradingView to begin continuous analysis.</div></div><div class="scoreWrap"><div id="ring" class="scoreRing" style="--score:0"><span id="ringScore">0</span></div><div class="scoreLabel">ENGINE SCORE</div></div></div><div class="scanline"></div><div class="levels"><div class="metric"><div class="lab">Price</div><div id="price" class="val">—</div></div><div class="metric"><div class="lab">Entry zone</div><div id="entry" class="val">—</div></div><div class="metric"><div class="lab">Invalidation</div><div id="stop" class="val">—</div></div><div class="metric"><div class="lab">Target 1</div><div id="tp1" class="val">—</div></div><div class="metric"><div class="lab">Target 2</div><div id="tp2" class="val">—</div></div></div><div class="contextGrid"><div class="context glass"><div class="lab">Market structure</div><div id="structure" class="val neutral">STANDBY</div></div><div class="context glass"><div class="lab">Macro / event risk</div><div id="macro" class="val neutral">STANDBY</div><div id="macrod" class="notice"></div></div><div class="context glass"><div class="lab">YM direction</div><div id="ym" class="val neutral">STANDBY</div></div><div class="context glass"><div class="lab">YM L1 order flow</div><div id="l2" class="val neutral">STANDBY</div><div id="l2d" class="notice"></div></div><div class="context glass"><div class="lab">Timeframe agreement</div><div id="alignment" class="val neutral">—</div><div id="alignDetail" class="notice"></div></div></div><div class="engineWhy"><div class="whyTitle">WHY THE ENGINE IS HERE</div><div id="why" class="whyText">Waiting for live market structure, momentum, volatility and context.</div></div><div class="statebar glass"><button id="sIDLE" disabled>SCANNING</button><button id="sLOOKING" disabled>BIAS IDENTIFIED</button><button id="sARMED" disabled>ENTRY READY</button><button id="sLIVE" onclick="startLive()">LIVE TRADE</button></div></div><div class="side glass"><div class="radar"><div class="radarSweep"></div><div class="radarCore"><div class="eyebrow">COPILOT READINESS</div><div id="readiness" class="readiness warn">SCANNING</div><div id="readyScore" class="notice">0 / 100</div></div></div><div id="readyDetail" class="notice">The engine will explain why a setup is or is not ready.</div><div><div class="lab">Active strategy</div><select id="strategy" class="field" onchange="saveStrategy()"><option value="scalp">Scalp · minutes–1h</option><option value="intraday">Intraday · 1–8h</option><option value="swing">Swing · days</option></select></div><div><div class="lab">Current trade</div><div id="tradeState" class="val">SCANNING</div><div id="tradeDetail" class="notice">Validation learning is always active.</div></div><div><div class="lab">Live P/L</div><div id="liveR" class="val">—</div></div></div></div><div class="chartCard glass"><div class="chartTop"><div><div class="eyebrow">LIVE MARKET MAP</div><div class="chartTitle">US30 price action + Copilot levels</div><div id="chartMeta" class="chartMeta">Waiting for candle history…</div><div class="chartTelemetry"><span><i></i>LIVE OHLC</span><span>SECURE FEED</span><span>ENGINE OVERLAY</span></div></div><div class="chartTf"><button class="on" data-tf="1m" onclick="setChartTf('1m')">1m</button><button data-tf="5m" onclick="setChartTf('5m')">5m</button><button data-tf="15m" onclick="setChartTf('15m')">15m</button><button data-tf="30m" onclick="setChartTf('30m')">30m</button><button data-tf="1h" onclick="setChartTf('1h')">1h</button><button data-tf="4h" onclick="setChartTf('4h')">4h</button></div></div><div class="chartWrap"><canvas id="candleCanvas"></canvas><div id="chartEmpty" class="chartEmpty">Waiting for TradingView candle history…</div></div><div class="chartLegend"><span><b>●</b> EMA 20</span><span><b>●</b> EMA 50</span><span>Copilot levels appear only when an entry-ready plan exists: entry zone · invalidation · TP1 · TP2.</span></div></div></div>
<div id="opportunitiesPage" class="hidden"><div class="section"><div class="sectionHead"><div><div class="eyebrow">OPPORTUNITY ENGINE</div><h1>Qualified setups, not noise.</h1><div class="notice">Copilot separates direction, readiness and execution timing.</div></div><div class="pill live">ALWAYS SCANNING</div></div><div class="pageGrid"><div class="premiumCard glass"><div class="lab">Current opportunity</div><div id="oppSignal" class="bigStat warn">SCANNING</div><div id="oppText" class="notice">No live opportunity yet.</div><div class="eventList"><div class="event"><div><strong>STRUCTURE</strong><br><span>Multi-timeframe market direction</span></div><b id="oppStructure">—</b></div><div class="event"><div><strong>CONTEXT</strong><br><span>Macro / economic event risk</span></div><b id="oppMacro">—</b></div><div class="event"><div><strong>FUTURES CONFIRMATION</strong><br><span>YM cross-market confirmation</span></div><b id="oppYm">—</b></div><div class="event"><div><strong>ENTRY READINESS</strong><br><span>Is the setup ready now?</span></div><b id="oppReady">—</b></div></div></div><div class="premiumCard glass"><div class="eyebrow">COPILOT PRINCIPLE</div><h2>Direction is not entry timing.</h2><p class="notice">The engine may hold a directional bias while still telling you to WAIT. That separation is deliberate: it reduces chasing and turns the dashboard into decision support rather than a stream of raw signals.</p><div class="engineWhy"><div class="whyTitle">CURRENT ENGINE EXPLANATION</div><div id="oppWhy" class="whyText">Connect your data feed to populate this panel.</div></div></div></div></div></div>
...
<div id="performancePage" class="hidden"><div class="section"><div class="eyebrow">ENGINE VALIDATION</div><h1>Measure the engine, don't believe the marketing.</h1><p class="notice">Engine validation is automatic and independent of whether you personally take a trade. Results are separated by strategy.</p><div class="perfGrid"><div class="perfStat glass"><div class="lab">Validation trades</div><div id="pTracked" class="bigStat">0</div></div><div class="perfStat glass"><div class="lab">Validation closed</div><div id="pClosed" class="bigStat">0</div></div><div class="perfStat glass"><div class="lab">TP1 hit rate</div><div id="pTp1" class="bigStat">—</div></div><div class="perfStat glass"><div class="lab">Expectancy</div><div id="pExp" class="bigStat">—</div></div></div><div class="premiumCard glass" style="margin-top:12px"><div class="lab">Manual live trades</div><div style="display:flex;gap:22px;margin-top:8px"><div><b id="pManualLive" style="font-size:24px">0</b><div class="small">Accepted with LIVE TRADE</div></div><div><b id="pManualClosed" style="font-size:24px">0</b><div class="small">Manually tracked closed</div></div></div><p class="notice">Manual live-trade tracking is separate from engine validation and begins only when you press LIVE TRADE and confirm your actual entry.</p></div><div class="premiumCard glass" style="margin-top:12px"><div class="lab">Validation status</div><div id="validationStatus" class="val warn">BUILDING SAMPLE</div><p class="notice">Confidence scores become meaningful only after enough closed engine setups. Sample size is shown before any performance claims.</p></div></div></div>
<div id="connectionsPage" class="hidden"><div class="section">
<div id="connectionsLocked" class="premiumCard glass hidden" style="margin-bottom:18px">
  <div class="eyebrow">SUBSCRIPTION REQUIRED</div>
  <h2 style="margin:6px 0 8px">Reactivate to manage data connections.</h2>
  <p class="notice">Your TradingView webhook and provider credentials are unavailable while BETA access is inactive.</p>
  <button class="btn" onclick="showPage('settings')">Go to Billing & Access</button>
</div>
<div id="connectionsContent"><div class="sectionHead"><div><div class="eyebrow">DATA CONNECTIONS</div><h1>Your data. One intelligence layer.</h1><p class="notice">Each account gets a private TradingView webhook. Optional provider keys are encrypted at rest using your server APP_SECRET.</p></div></div>
<div class="coverageHero glass">
  <div id="coverageBrain" class="coverageBrain" data-level="1">
    <img src="/data-brain.png" alt="US30 Copilot data brain">
    <div class="coveragePct"><div><b id="coveragePct">33.3%</b><span>INPUT COVERAGE</span></div></div>
  </div>
  <div>
    <div class="eyebrow">COPILOT DATA BRAIN</div>
    <h2 id="coverageLabel" style="font-size:23px;margin:6px 0 4px">TECHNICAL CORE</h2>
    <div id="coverageText" class="coverageNote">TradingView powers the core technical engine. Add the free FRED key for macro context; Databento adds the final L1 order-flow confirmation layer.</div>
    <div class="coverageBar"><i id="coverageFill"></i></div>
    <div class="coverageSources">
      <div id="covTv" class="coverageSource on"><b>TradingView · 33.3%</b><span>Technical structure + multi-timeframe analysis</span></div>
      <div id="covFred" class="coverageSource"><b>FRED · +33.3%</b><span>Free macro + rates context</span></div>
      <div id="covDb" class="coverageSource"><b>Databento · +33.4%</b><span>Optional YM L1 order-flow confirmation</span></div>
    </div>
    <div class="coverageNote" style="margin-top:11px"><b>Input Coverage is not Trade Readiness.</b> More connected sources increase confirmation depth; the engine still waits for a qualified market setup.</div>
  </div>
</div>
<div class="connections"><div class="conn glass"><div class="lab"><span id="tvDot" class="dot on"></span>TradingView</div><div class="val">PERSONAL WEBHOOK</div><div id="webhook" class="copy">Loading…</div><button class="btn" onclick="copyWebhook()" style="margin-top:8px">Copy webhook</button><button class="btn ghost" onclick="rotateWebhook()" style="margin-top:8px">Rotate URL</button><div class="notice" style="margin-top:9px">Primary live technical / multi-timeframe feed.</div></div><div class="conn glass"><div class="lab"><span id="fredDot" class="dot"></span>FRED Macro</div><div class="val">OPTIONAL · FREE KEY</div><input id="fredKey" class="field" type="password" placeholder="Paste FRED API key"><button class="btn" onclick="saveConnections()" style="margin-top:8px">Save securely</button><div class="notice">Rates and macro-regime context.</div></div><div class="conn glass"><div class="lab"><span id="dbDot" class="dot"></span>Databento</div><div class="val">OPTIONAL · LICENSED L1</div><input id="dbKey" class="field" type="password" placeholder="Paste Databento API key"><button class="btn" onclick="saveConnections()" style="margin-top:8px">Save securely</button><div class="notice">Prepared for licensed CME L1 order-flow confirmation.</div></div></div><div class="wizard glass"><div class="wizardHead"><div><div class="eyebrow">TRADINGVIEW SETUP WIZARD</div><h2 style="margin:5px 0 0">Connect in five steps</h2><p class="notice">The webhook is the destination. The Pine feed calculates and sends the actual market-data packet.</p></div><div id="wizardStatus" class="pill">NOT VERIFIED</div></div><div class="tvProgress"><div class="tvProgressTop"><b id="tvProgressLabel">TradingView setup · 0 / 5 complete</b><span id="tvProgressHint">About 3 minutes</span></div><div class="tvProgressBar"><i id="tvProgressFill"></i></div></div><div class="wizardSteps"><div id="tvStep1" class="wizardStep"><div class="stepNo">1</div><div><div class="stepTitle">Choose your TradingView feed</div><div class="stepText">For BETA, choose <b>Standard</b> unless you already pay TradingView for CME/CBOT futures data. You can change this later.</div><div class="feedChoice"><div id="feedStandard" class="feedCard on" onclick="chooseFeed('standard')"><b>Standard</b><div class="stepText">US30 multi-timeframe feed</div></div><div id="feedCme" class="feedCard" onclick="chooseFeed('cme')"><b>CME Enhanced</b><div class="stepText">US30 + YM futures confirmation</div></div></div><div class="wizardActions"><a id="pineDownload" class="btn" href="/downloads/tradingview-pine?feed=standard" onclick="markTvStep(1)">Download Pine code</a></div><div class="tvCallout">Recommended for most BETA users: <b>Standard</b>. FRED and Databento are configured separately in Copilot.</div></div></div><div id="tvStep2" class="wizardStep"><div class="stepNo">2</div><div><div class="stepTitle">Install the Pine feed in TradingView</div><div class="stepText"><b>The download does not auto-install into TradingView.</b> In TradingView: open your US30/Dow chart → set chart to <b>1 minute</b> → open <b>Pine Editor</b> → create a blank indicator. Press <b>Copy Pine code</b> below, paste it into Pine Editor, then <b>Save → Add to chart</b>.</div><div class="wizardActions"><button class="btn ghost" onclick="copyPineCode()">Copy selected Pine code</button></div><div id="pineCopyStatus" class="notice" style="margin-top:7px">Tip: use this button to avoid broken or partial copy/paste.</div></div></div><div id="tvStep3" class="wizardStep"><div class="stepNo">3</div><div><div class="stepTitle">Create one TradingView alert</div><div class="stepText">In TradingView press <b>Create Alert</b>. For Condition choose the US30 Copilot script, then choose <b>Any alert() function call</b>. Set Frequency to <b>Once Per Bar Close</b>. You only need one alert.</div><div id="conditionHint" class="setupCode">US30 COPILOT 6.4 LIVE FEED → Any alert() function call</div><div class="wizardActions"><button class="btn ghost" onclick="markTvStep(3)">I created the alert</button></div></div></div><div id="tvStep4" class="wizardStep"><div class="stepNo">4</div><div><div class="stepTitle">Paste your personal webhook</div><div class="stepText">Open the alert's <b>Notifications</b> tab → enable <b>Webhook URL</b> → paste your private address below. <b>Do not edit the Message field</b>; the Copilot Pine script creates the JSON automatically.</div><div id="wizardWebhook" class="setupCode">Loading…</div><div class="wizardActions"><button class="btn" onclick="copyWebhook()">Copy webhook</button></div></div></div><div id="tvStep5" class="wizardStep"><div class="stepNo">5</div><div><div class="stepTitle">Verify the connection</div><div class="stepText">Save the alert, then wait for the next completed 1-minute candle. Copilot checks the feed automatically. A fresh packet completes setup.</div><div id="verifyBox" class="verifyBox notice">Waiting for a TradingView packet.</div><div class="wizardActions"><button class="btn" onclick="verifyTradingView()">Check live feed</button></div></div></div></div></div></div></div></div><div id="settingsPage" class="hidden"><div class="section"><div class="eyebrow">PERSONALISE COPILOT</div><h1>Only notify what matters.</h1><div class="settingsGrid"><div class="premiumCard glass"><div class="lab">Trading mode</div><select id="strategy2" class="field" onchange="syncStrategy()"><option value="scalp">Scalp · minutes–1h</option><option value="intraday">Intraday · 1–8h</option><option value="swing">Swing · days</option></select><div class="engineWhy" style="margin-top:12px"><div class="whyTitle">PHONE / BACKGROUND PUSH</div><div id="pushStatus" class="whyText">Not enabled on this device.</div><div class="wizardActions"><button class="btn" onclick="enablePush()">Enable background push</button><button class="btn ghost" onclick="testPush()">Send test</button></div></div><div class="toggleLine"><label>Setup developing</label><input id="nsetup" type="checkbox" onchange="saveAlerts()"></div><div class="toggleLine"><label>Entry ready</label><input id="nready" type="checkbox" onchange="saveAlerts()"></div><div class="toggleLine"><label>Live trade updates</label><input id="nupdate" type="checkbox" onchange="saveAlerts()"></div><div class="toggleLine"><label>Exit-risk warning</label><input id="nexit" type="checkbox" onchange="saveAlerts()"></div><div class="toggleLine"><label>Macro / event warning</label><input id="nnews" type="checkbox" onchange="saveAlerts()"></div><div class="toggleLine"><label>Auto-arm qualified setups</label><input id="autoarm" type="checkbox" onchange="saveAlerts()"></div></div><div class="premiumCard glass"><div class="eyebrow">ACCOUNT</div><div id="accountName" class="val">—</div><p class="notice">Your personal webhook and provider credentials belong to this account only.</p><div class="engineWhy" style="margin-top:12px"><div class="whyTitle">BILLING & ACCESS</div><div id="billingStatus" class="whyText">Checking subscription…</div><div class="wizardActions"><button id="subscribeBtn" class="btn" onclick="startCheckout()">Subscribe · £29.99/month</button><button id="portalBtn" class="btn ghost hidden" onclick="openBillingPortal()">Manage subscription</button></div><div class="small" style="margin-top:9px"><b>Beta data requirement:</b> Third-party data is not included. You need your own compatible TradingView access and your own Databento subscription/API key. FRED can use a free API key.</div></div><button class="btn ghost" style="margin-top:12px" onclick="logout()">Sign out</button><div class="engineWhy" style="margin-top:14px"><div class="whyTitle">PRODUCT POSITIONING</div><div class="whyText">US30 Copilot is a data-engine decision-support product. It explains the inputs and reasoning behind a setup rather than presenting itself as a guaranteed signal service.</div></div></div></div></div></div>
...
let pad={l:16,r:118,t:24,b:30},pw=w-pad.l-pad.r,ph=h-pad.t-pad.b,d=latest||{},plan=copilot||{};
let assessment=currentAssessment||{},entryReady=assessment.quality==='READY'&&assessment.inside_entry&&(d.signal==='LONG'||d.signal==='SHORT');
let planned=(plan.status==='ARMED'||plan.status==='LIVE')&&plan.planned_entry_low!=null&&plan.planned_entry_high!=null;
let showPlan=entryReady||planned,overlay={entry_low:planned?plan.planned_entry_low:d.entry_low,entry_high:planned?plan.planned_entry_high:d.entry_high,stop:planned?plan.stop:d.stop,tp1:planned?plan.tp1:d.tp1,tp2:planned?plan.tp2:d.tp2,side:planned?(plan.side||d.signal):d.signal};
let levels=[];if(showPlan){['entry_low','entry_high','stop','tp1','tp2'].forEach(k=>{let v=Number(overlay[k]);if(Number.isFinite(v)&&v>0)levels.push(v)})}
...
x.strokeStyle='rgba(106,154,191,.09)';x.lineWidth=1;x.font='500 10px ui-monospace,monospace';x.fillStyle='#647e96';x.textAlign='left';for(let i=0;i<=5;i++){let yy=pad.t+i*ph/5,v=hi-i*span/5;x.beginPath();x.moveTo(pad.l,yy+.5);x.lineTo(w-pad.r,yy+.5);x.stroke();x.fillText(v.toLocaleString(undefined,{maximumFractionDigits:1}),w-pad.r+9,yy+3)}
for(let i=0;i<=5;i++){let xx=pad.l+i*pw/5;x.beginPath();x.moveTo(xx+.5,pad.t);x.lineTo(xx+.5,h-pad.b);x.stroke()}
let maxVol=Math.max(1,...bars.map(b=>Number(b.vol)||0));bars.forEach((b,i)=>{let vol=Number(b.vol)||0;if(!vol)return;let vh=Math.max(1,(vol/maxVol)*ph*.16),xx=px(i);x.fillStyle=b.c>=b.o?'rgba(49,230,160,.10)':'rgba(255,102,122,.10)';x.fillRect(xx-Math.max(1,pw/bars.length*.31),h-pad.b-vh,Math.max(2,pw/bars.length*.62),vh)});
let bw=Math.max(2.5,Math.min(10,pw/bars.length*.66));bars.forEach((b,i)=>{let xx=px(i),yo=py(b.o),yc=py(b.c),yh=py(b.h),yl=py(b.l),up=b.c>=b.o,bodyTop=Math.min(yo,yc),bodyH=Math.max(1.5,Math.abs(yc-yo));x.strokeStyle=up?'#4ee6b0':'#ff667d';x.lineWidth=1.2;x.beginPath();x.moveTo(xx,yh);x.lineTo(xx,yl);x.stroke();let g=x.createLinearGradient(0,bodyTop,0,bodyTop+bodyH);if(up){g.addColorStop(0,'#52efbc');g.addColorStop(1,'#16b985')}else{g.addColorStop(0,'#ff7288');g.addColorStop(1,'#d94760')}x.fillStyle=g;x.fillRect(xx-bw/2,bodyTop,bw,bodyH);x.strokeRect(xx-bw/2+.5,bodyTop+.5,Math.max(0,bw-1),Math.max(0,bodyH-1))});
function lineSeries(vals,color,width){x.save();x.strokeStyle=color;x.lineWidth=width;x.shadowColor=color;x.shadowBlur=3;x.beginPath();vals.forEach((v,i)=>{let xx=px(i),yy=py(v);if(i===0)x.moveTo(xx,yy);else x.lineTo(xx,yy)});x.stroke();x.restore()}lineSeries(emaSeries(bars,20),'rgba(45,195,255,.78)',1.35);lineSeries(emaSeries(bars,50),'rgba(255,190,79,.72)',1.25);
...
if(showPlan&&Number.isFinite(Number(overlay.entry_low))&&Number.isFinite(Number(overlay.entry_high))){let el=Number(overlay.entry_low),eh=Number(overlay.entry_high),top=py(Math.max(el,eh)),bot=py(Math.min(el,eh)),zg=x.createLinearGradient(0,top,0,bot);zg.addColorStop(0,overlay.side==='LONG'?'rgba(49,230,160,.18)':'rgba(255,102,122,.18)');zg.addColorStop(1,'rgba(29,170,235,.035)');x.fillStyle=zg;x.fillRect(pad.l,top,pw,Math.max(4,bot-top));level(el,'ENTRY LOW','#4acbff',[4,4]);level(eh,'ENTRY HIGH','#4acbff',[4,4]);level(overlay.stop,'INVALID','#ff667a',[8,4],1.8);level(overlay.tp1,'TP1','#31e6a0',[5,4],1.7);level(overlay.tp2,'TP2','#31e6a0',[5,4],1.7)}
let cp=Number(bars[bars.length-1].c);level(cp,'PRICE','rgba(224,240,255,.90)',[2,4],1.25);
let lastX=px(bars.length-1),lastY=py(cp);x.save();x.fillStyle='#dff6ff';x.shadowColor='#55d8ff';x.shadowBlur=12;x.beginPath();x.arc(lastX,lastY,2.7,0,Math.PI*2);x.fill();x.restore();
...
labels.forEach(q=>{let text=q.label+'  '+q.v.toLocaleString(undefined,{maximumFractionDigits:1});x.save();x.strokeStyle='rgba(102,156,202,.32)';x.lineWidth=1;x.beginPath();x.moveTo(w-pad.r,q.y);x.lineTo(w-pad.r+8,q.ly);x.lineTo(w-pad.r+12,q.ly);x.stroke();x.font='800 9px ui-monospace,monospace';let tw=Math.min(pad.r-17,x.measureText(text).width+14);x.fillStyle='rgba(3,12,22,.96)';x.beginPath();x.roundRect(w-pad.r+12,q.ly-9,tw,18,4);x.fill();x.strokeStyle=q.color;x.stroke();x.fillStyle=q.color;x.textAlign='left';x.fillText(text,w-pad.r+18,q.ly+3);x.restore()});
...
function drawAuth(){let reset=mode==='reset';$('name').style.display=mode==='register'?'block':'none';$('email').classList.toggle('hidden',reset);$('password2').classList.toggle('hidden',!reset);$('forgotBtn').classList.toggle('hidden',mode!=='login');$('backLoginBtn').classList.toggle('hidden',!reset);$('authBtn').textContent=reset?'Set new password':mode==='register'?'Create account':'Sign in';$('authErr').textContent=''}
function backToLogin(){history.replaceState({},'',location.pathname);mode='login';$('password').value='';$('password2').value='';drawAuth()}
async function forgotPassword(){let email=$('email').value.trim();if(!email){$('authErr').textContent='Enter your email address first.';return}let r=await fetch('/auth/forgot-password',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email})});let j=await r.json();$('authErr').textContent=j.message||'If that email is registered, a reset link has been sent.'}
async function authSubmit(){if(mode==='reset'){let token=new URLSearchParams(location.search).get('reset_token')||'',p=$('password').value,p2=$('password2').value;if(p!==p2){$('authErr').textContent='Passwords do not match.';return}let r=await fetch('/auth/reset-password',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({token,password:p})}),j=await r.json();if(!r.ok){$('authErr').textContent=j.detail||'Unable to reset password';return}history.replaceState({},'',location.pathname);mode='login';$('password').value='';$('password2').value='';drawAuth();$('authErr').textContent='Password updated. Sign in with your new password.';return}let url=mode==='register'?'/auth/register':'/auth/login',body={email:$('email').value,password:$('password').value,display_name:$('name').value};let r=await fetch(url,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});let j=await r.json();if(!r.ok){$('authErr').textContent=j.detail||'Unable to continue';return}await boot()}
...
{icon:'◎',title:'Engine validation runs automatically',body:'When an ARMED setup becomes entry-ready, the engine records that setup for objective performance validation automatically. This happens whether you personally take the trade or not.',check:'Performance → Validation Trades measures the engine. It is separate from your own trading results.',action:'performance'},
...
{icon:'✓',title:'You’re ready',body:'Copilot scans and validates continuously. Qualified setups automatically progress from directional bias to entry ready. Wait for the entry-ready alert, make your own trade decision, and press LIVE TRADE only if you actually enter.',check:'The engine learns its statistical performance automatically; your manual live-trade history remains a separate record.',action:null}
];
...
function displayPlanName(p){return String(p||'BETA').toUpperCase()==='ALPHA'?'FOUNDER':String(p||'BETA').toUpperCase()}
function renderBilling(b){if(!b)return;let st=$('billingStatus'),sub=$('subscribeBtn'),portal=$('portalBtn'),planName=displayPlanName(b.plan);let label=planName+' · '+String(b.status||'inactive').toUpperCase();if(st)st.textContent=label+(b.current_period_end?' · current period to '+new Date(b.current_period_end).toLocaleDateString():'');if(sub)sub.classList.toggle('hidden',!!b.entitled);if(portal)portal.classList.toggle('hidden',!b.entitled||b.status==='grandfathered');$('plan').textContent=planName+(b.entitled?' · ACTIVE':'')}
...
async function boot(){let resetToken=new URLSearchParams(location.search).get('reset_token');if(resetToken){mode='reset';$('login').classList.remove('hidden');$('app').classList.add('hidden');drawAuth();return}let r=await fetch('/auth/me');if(!r.ok){$('login').classList.remove('hidden');$('app').classList.add('hidden');drawAuth();return}let j=await r.json();user=j.user;$('login').classList.add('hidden');$('app').classList.remove('hidden');$('plan').textContent=displayPlanName(user.plan);$('accountName').textContent=(user.display_name||user.email)+' · '+displayPlanName(user.plan);renderBilling(j.billing);if(j.billing&&!j.billing.entitled){document.querySelectorAll('.nav button').forEach(b=>{if(!['settings','connections'].includes(b.dataset.page))b.disabled=true});showPage('settings');setTimeout(()=>toast('Subscribe to unlock the live decision engine'),250)}await load();await loadConnections();refreshPushStatus();refreshBilling();let qp=new URLSearchParams(location.search);if(qp.get('billing')==='success'){toast('Subscription checkout completed');history.replaceState({},'',location.pathname);setTimeout(refreshBilling,1200)}else if(qp.get('billing')==='cancelled'){toast('Checkout cancelled');history.replaceState({},'',location.pathname)}maybeStartTour();clearInterval(loadTimer);clearInterval(notifTimer);loadTimer=setInterval(load,5000);notifTimer=setInterval(pollNotifications,5000)}
...
function render(d,received,a){latest=d;currentAssessment=a||{};feed(received);let directional=d.signal==='LONG'||d.signal==='SHORT',entryReady=directional&&a?.quality==='READY'&&a?.inside_entry,display=entryReady?d.signal+' · ENTRY READY':directional?d.signal+' BIAS':'SCANNING';$('symbol').textContent=(d.symbol||'US30')+' · '+d.strategy_label+' · '+d.horizon;$('signal').textContent=display;$('signal').className='sig '+(directional?d.signal:'WAIT');$('confidence').textContent=entryReady?'ENTRY CONDITIONS CONFIRMED':directional?'WAITING FOR ENTRY · NOT AN ENTRY':'Continuous validation scanning';$('summary').textContent='Alignment '+d.alignment+'/'+d.alignment_total+' · weighted model '+d.weighted_score;$('ring').style.setProperty('--score',directional?d.confidence:Math.min(60,d.confidence||0));$('ringScore').textContent=(d.confidence||0);$('price').textContent=fmt(d.price);$('entry').textContent=entryReady&&d.entry_low!=null?fmt(d.entry_low)+'–'+fmt(d.entry_high):'—';$('stop').textContent=entryReady?fmt(d.stop):'—';$('tp1').textContent=entryReady?fmt(d.tp1):'—';$('tp2').textContent=entryReady?fmt(d.tp2):'—';$('structure').textContent=directional?d.signal+' BIAS':'MIXED';$('structure').className='val '+(d.signal==='LONG'?'good':d.signal==='SHORT'?'bad':'warn');let n=d.news||{};$('macro').textContent=n.risk||'LOW';$('macro').className='val '+(n.risk==='HIGH'?'bad':n.risk==='WATCH'?'warn':'good');$('macrod').textContent=(n.macro?.available?'FRED regime active':'Federal Reserve calendar active');let y=d.ym||{};$('ym').textContent=y.available?(y.score>0?'BULLISH CONFIRM':y.score<0?'BEARISH CONFIRM':'NEUTRAL'):'NO CONFIRMATION';$('ym').className='val '+(y.score>0?'good':y.score<0?'bad':'warn');let o=d.orderflow||{};$('l2').textContent=o.available?(o.score>.35?'BUY PRESSURE':o.score<-.35?'SELL PRESSURE':'BALANCED'):(o.configured?'WAITING':'NOT CONNECTED');$('l2').className='val '+(o.available?(o.score>.35?'good':o.score<-.35?'bad':'warn'):'warn');$('l2d').textContent=o.available?('L1 imbalance '+Number(o.depth_imbalance||0).toFixed(2)+' · trade delta '+Number(o.delta_norm||0).toFixed(2)+' · '+Number(o.age_seconds||0).toFixed(1)+'s'):(o.error||o.status||'');$('alignment').textContent=(d.alignment||0)+' / '+(d.alignment_total||0);$('alignDetail').textContent=d.alignment_total?'multi-timeframe directional agreement':'';$('why').textContent=reasonsText(d,a);if(a){$('readiness').textContent=entryReady?'ENTRY READY':directional?'WAITING':'SCANNING';$('readyScore').textContent=a.score+'/100';$('readiness').className='readiness '+(entryReady?'good':a.quality==='BLOCKED'?'bad':'warn');$('readyDetail').textContent=entryReady?'All entry conditions are confirmed now.':(a.reasons||[]).slice(0,4).join(' · ')||'Scanning for a qualified setup.'}renderAux();drawCandles()}
function renderCopilot(c){copilot=c;let st=c?.status||'LOOKING',directional=latest&&(latest.signal==='LONG'||latest.signal==='SHORT'),entryReady=directional&&currentAssessment?.quality==='READY'&&currentAssessment?.inside_entry,uiState=st==='LIVE'?'LIVE':entryReady?'ARMED':directional?'LOOKING':'IDLE';['IDLE','LOOKING','ARMED','LIVE'].forEach(x=>$('s'+x).className=''+(x===uiState?'active':''));$('tradeState').textContent=st==='LIVE'?'TRADE ACTIVE':entryReady?'ENTRY READY':directional?'WAITING FOR ENTRY':'SCANNING';$('tradeDetail').textContent=st==='LIVE'?(c.side+' · entry '+fmt(c.actual_entry)):entryReady?((c?.side||latest.signal)+' · entry conditions confirmed'):directional?(latest.signal+' bias · not an entry'):'Validation learning is always active.';$('liveR').textContent=st==='LIVE'?(Number(c.last_r||0)>=0?'+':'')+Number(c.last_r||0).toFixed(2)+'R':'—';renderAux()}
function renderAux(){let d=latest||{},n=d.news||{},y=d.ym||{},c=copilot||{},st=c.status||'LOOKING',directional=d.signal==='LONG'||d.signal==='SHORT',entryReady=directional&&currentAssessment?.quality==='READY'&&currentAssessment?.inside_entry;$('oppSignal').textContent=entryReady?d.signal+' · ENTRY READY':directional?d.signal+' BIAS':'SCANNING';$('oppSignal').className='bigStat '+(d.signal==='LONG'?'good':d.signal==='SHORT'?'bad':'warn');$('oppText').textContent=entryReady?'Entry conditions are confirmed now.':directional?'Directional bias identified. Waiting for entry confirmation — do not enter yet.':'Continuously scanning for a qualified directional setup.';$('oppStructure').textContent=directional?d.signal+' BIAS':'MIXED';$('oppMacro').textContent=n.risk||'—';$('oppYm').textContent=y.available?(y.score>0?'BULLISH':y.score<0?'BEARISH':'NEUTRAL'):'—';$('oppReady').textContent=entryReady?'ENTRY READY':directional?'WAITING':'SCANNING';$('oppWhy').textContent=$('why').textContent||'—';$('liveSide').textContent=st==='LIVE'?(c.side||'LIVE'):'NO ACTIVE TRADE';$('liveSide').className='tradeSide '+(c.side==='LONG'?'good':c.side==='SHORT'?'bad':'warn');$('liveMeta').textContent=st==='LIVE'?'Copilot is monitoring structure, risk and targets.':'Engine validation continues in the background. Press “I’m in” only after you actually enter.';$('livePageR').textContent=st==='LIVE'?((Number(c.last_r||0)>=0?'+':'')+Number(c.last_r||0).toFixed(2)+'R'):'—';$('livePageR').className='rBig '+(Number(c.last_r||0)>=0?'good':'bad');$('ltEntry').textContent=fmt(c.actual_entry);$('ltPrice').textContent=fmt(c.last_price||d.price);$('ltStop').textContent=fmt(c.stop);$('ltTp1').textContent=fmt(c.tp1);$('ltTp2').textContent=fmt(c.tp2);let health=st==='LIVE'?Math.max(8,Math.min(100,50+Number(c.last_r||0)*22)):0;$('healthFill').style.width=health+'%';$('healthText').textContent=st==='LIVE'?(Number(c.last_r||0)>=1?'Trade progressing favourably. Continue monitoring structure and event risk.':Number(c.last_r||0)<-.35?'Trade under pressure. Re-check invalidation and structure.':'Position active. No major trade-health conclusion yet.'):'No manual position is being monitored; autonomous validation remains live.'}
...
async function load(){try{let j=await(await fetch('/api/latest',{cache:'no-store'})).json();if(!j.ok)return;applySettings(j.settings);currentAssessment=j.entry_assessment||{};if(j.data)render(j.data,j.received_at,j.entry_assessment);else{latest=null;feed(null);renderAux()}renderCopilot(j.copilot);document.querySelectorAll('.chartTf button').forEach(b=>b.classList.toggle('on',b.dataset.tf===chartTf));await loadCandles(false)}catch(e){}}
...
async function copyPineCode(){try{let r=await fetch('/downloads/tradingview-pine?feed='+setupFeed,{cache:'no-store'});if(!r.ok)throw new Error('download failed');let t=await r.text();if(!t.trim().startsWith('//@version=6'))throw new Error('Pine file failed integrity check');await navigator.clipboard.writeText(t);$('pineCopyStatus').textContent='Copied complete Pine file to clipboard — first line verified as //@version=6';markTvStep(2);toast('Pine code copied')}catch(e){$('pineCopyStatus').textContent='Copy failed: '+e.message}}
...
function tvSetupKey(){return 'us30TvSetupV2:'+(user?.id||user?.email||'guest')}
function tvSetupState(){try{return JSON.parse(localStorage.getItem(tvSetupKey())||'{}')}catch(_){return {}}}
function saveTvSetupState(st){localStorage.setItem(tvSetupKey(),JSON.stringify(st));renderTvProgress()}
function markTvStep(n){let st=tvSetupState();st['s'+n]=true;saveTvSetupState(st)}
function renderTvProgress(){let st=tvSetupState(),verified=!!st.s5;let done=[1,2,3,4,5].filter(n=>st['s'+n]).length;let lab=$('tvProgressLabel'),fill=$('tvProgressFill'),hint=$('tvProgressHint');if(lab)lab.textContent='TradingView setup · '+done+' / 5 complete';if(fill)fill.style.width=(done*20)+'%';if(hint)hint.textContent=verified?'Live feed verified':'Complete each step in order';for(let n=1;n<=5;n++){let el=$('tvStep'+n);if(el)el.classList.toggle('done',!!st['s'+n])}}
function chooseFeed(feed){markTvStep(1);setupFeed=feed;localStorage.setItem('setupFeed',feed);let a=$('pineDownload');if(a)a.href='/downloads/tradingview-pine?feed='+feed;let st=$('feedStandard'),cm=$('feedCme');if(st)st.className='feedCard '+(feed==='standard'?'on':'');if(cm)cm.className='feedCard '+(feed==='cme'?'on':'');let h=$('conditionHint');if(h)h.textContent=(feed==='cme'?'US30 LIVE DATA FEED V6 + CME YM':'US30 COPILOT 6.4 LIVE FEED')+' → Any alert() function call'}
async function loadConnections(){let r=await fetch('/api/connections',{cache:'no-store'});if(!r.ok){if(r.status===402||r.status===403){conn=null;let lock=$('connectionsLocked'),content=$('connectionsContent');if(lock)lock.classList.remove('hidden');if(content)content.classList.add('hidden');$('webhook').textContent='Subscription required';let ww=$('wizardWebhook');if(ww)ww.textContent='Subscription required';return}throw new Error('Connections unavailable: '+r.status)}let j=await r.json();conn=j.connections;let lock=$('connectionsLocked'),content=$('connectionsContent');if(lock)lock.classList.add('hidden');if(content)content.classList.remove('hidden');let cv=conn.input_coverage||{tradingview:33.3,fred:conn.fred?.configured?33.3:0,databento:conn.databento?.configured?33.4:0,total:33.3,label:'TECHNICAL CORE'};let level=cv.databento>0?3:(cv.fred>0?2:1);$('coverageBrain').dataset.level=String(level);$('coveragePct').textContent=Number(cv.total||33.3).toFixed(1)+'%';$('coverageFill').style.width=Math.max(0,Math.min(100,Number(cv.total||33.3)))+'%';$('coverageLabel').textContent=cv.label||'TECHNICAL CORE';$('coverageText').textContent=level===3?'Full input stack connected: technical structure, macro context and YM L1 order-flow confirmation.':level===2?'TradingView + FRED connected: core technical analysis with macro/rates context. Databento remains optional for deeper order-flow confirmation.':'TradingView technical core connected. Add the free FRED key for macro/rates context; Databento remains optional.';$('covTv').classList.add('on');$('covFred').classList.toggle('on',cv.fred>0);$('covDb').classList.toggle('on',cv.databento>0);$('webhook').textContent=conn.tradingview.webhook_url;let ww=$('wizardWebhook');if(ww)ww.textContent=conn.tradingview.webhook_url;$('fredDot').className='dot '+(conn.fred.configured?'on':'');$('dbDot').className='dot '+(conn.databento.live?'on':'');let ds=$('dbStatus');if(ds){let s=conn.databento.status||{};ds.textContent=conn.databento.live?'LIVE · YM.FUT · L1 (MBP-1)':(conn.databento.configured?((s.status||'CONNECTING')+' · YM.FUT · L1 (MBP-1)'):'Not configured');}chooseFeed(setupFeed);renderTvProgress();verifyTradingView(false)}
...
async function copyWebhook(){try{let raw=(conn?.tradingview?.webhook_url||$('webhook').textContent||'');let w=String(raw).trim().replace(/[\u0000-\u001F\u007F-\u009F\u200B-\u200D\u2060\uFEFF]/g,'');let u=new URL(w);if(u.protocol!=='https:'||!u.hostname||!/^\/webhook\/tradingview\/[0-9a-fA-F]+$/.test(u.pathname))throw new Error('invalid webhook URL');w=u.origin+u.pathname;await navigator.clipboard.writeText(w);$('webhook').textContent=w;let ww=$('wizardWebhook');if(ww)ww.textContent=w;markTvStep(4);toast('Clean webhook copied')}catch(e){console.error(e);toast('Webhook copy failed: '+e.message)}}
async function verifyTradingView(showToast=true){try{let j=await(await fetch('/api/latest',{cache:'no-store'})).json();let box=$('verifyBox'),st=$('wizardStatus');if(!box||!st)return;if(j.data&&j.received_at){let age=Math.max(0,Math.round((Date.now()-new Date(j.received_at).getTime())/1000));if(age<180){markTvStep(5);box.innerHTML='<div class="tvSuccess">✓ TRADINGVIEW CONNECTED<br>✓ LIVE US30 DATA RECEIVED<br>✓ MULTI-TIMEFRAME FEED ACTIVE</div><div class="small" style="margin-top:6px">Last packet '+age+'s ago · '+(j.data.symbol||'US30')+'</div>'}else{box.textContent='TradingView was connected, but the last packet is '+age+'s old. Check that the TradingView alert is still enabled.'}box.className='verifyBox '+(age<180?'good':'notice');st.textContent=age<180?'CONNECTED':'STALE';st.className='pill '+(age<180?'live':'');if(showToast)toast(age<180?'TradingView live feed verified':'TradingView feed is stale')}else{box.innerHTML='<b>NO DATA RECEIVED YET</b><div class="tvTrouble">Check these three things:<br>1. The Copilot Pine indicator is added to a <b>1-minute US30/Dow chart</b>.<br>2. Your alert condition is <b>Any alert() function call</b> and the alert is enabled.<br>3. The alert Webhook URL exactly matches your private Copilot webhook above.</div>';box.className='verifyBox notice';st.textContent='WAITING FOR DATA';st.className='pill';if(showToast)toast('Still waiting for TradingView data')}}catch(e){if(showToast)toast('Connection check failed')}}
